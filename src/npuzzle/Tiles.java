...
public class Tiles implements State {
	...
	public boolean equals(Object that) {
		// objects are identical -> return true
		if (this == that) return true;
		// cannot be equal to null
		if (that == null) return false;
		// must be instances of the same class
		if (getClass() != that.getClass()) return false;
		// now that can be casted
		Tiles otherTiles = (Tiles) that;
		// tiles are equal if content of 'tiles'array is exactly the same
		return this.width == otherTiles.width && Arrays.equals(this.tiles, otherTiles.tiles);
	}

	/**
	 * Overriding hashCode in order to use HashSet<Tiles>
	 * @return
	 */
	@Override
  public int hashCode() {
